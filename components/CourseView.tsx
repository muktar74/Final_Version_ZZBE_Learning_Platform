...
const CourseView: React.FC<CourseViewProps> = ({ course, setCourses, currentUser, userProgress, onModuleComplete, onCourseComplete, onBack, addToast, onEnroll }) => {
  ...
  const getQuizButton = () => {
    if (!hasQuiz) {
        return <button disabled className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-white transition bg-slate-400 cursor-not-allowed">Quiz Not Available</button>;
    }
    if (hasCompletedCourse) {
        return <button disabled className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-white transition bg-green-500 cursor-not-allowed flex items-center justify-center space-x-2"><CheckCircleIcon className="h-5 w-5"/><span>Course Completed</span></button>;
    }
    if (!allModulesCompleted) {
        return <button disabled className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-white transition bg-slate-300 cursor-not-allowed flex items-center justify-center space-x-2"><LockClosedIcon className="h-5 w-5"/><span>Complete modules to unlock</span></button>;
    }
    
    const buttonText = hasTakenQuiz ? "Retake Final Quiz" : "Start Final Quiz";
    return <button onClick={() => setShowQuiz(true)} className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-white transition bg-zamzam-teal-600 hover:bg-zamzam-teal-700">{buttonText}</button>;
  };
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <aside className="lg:w-1/3 border-r-0 lg:border-r lg:pr-8 flex-shrink-0">
          ...
          <ul className="space-y-2">
           ...
           {course.textbookUrl && <a href={course.textbookUrl} download={course.textbookName} target="_blank" rel="noopener noreferrer" className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-zamzam-teal-800 bg-zamzam-teal-100 hover:bg-zamzam-teal-200 transition flex items-center justify-center space-x-2"><DownloadIcon className="h-5 w-5"/><span>Download Textbook</span></a>}
          {isEnrolled ? getQuizButton() : (
            <button onClick={() => onEnroll(course.id)} className="w-full mt-4 py-3 px-4 rounded-lg font-semibold text-white transition bg-zamzam-teal-600 hover:bg-zamzam-teal-700">Enroll in Course</button>
          )}
        </aside>
          ...
          {activeTab === 'content' && (
             activeModule && isEnrolled ? (
                <div>
                    ...
                    {activeModule.type === 'video' ? <div className="bg-black rounded-lg overflow-hidden shadow-sm"><VideoPlayer src={activeModule.content} title={activeModule.title} type={activeModule.videoType}/></div>
